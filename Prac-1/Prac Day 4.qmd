...
```{r}
print("The following was rewritten using dplyr and the pipe:")
...
```{r}
print("Below is the sd and mean distance travelled per carrier:")
flights |>
  filter(month == 1) |>
  group_by(carrier) |>
  summarise(
...
carrier_delay_df <- flights |>
  ...
  summarise(avg_dep_delay = mean(dep_delay, na.rm = TRUE), .groups = "drop") |> # use when summarising after using group_by to prevent results from still being partially grouped
  pivot_wider(names_from = carrier, values_from = avg_dep_delay)
 print("Here is the requested dataframe:")
print(carrier_delay_df)
...
delayed_timely <- flights |>
  ...
  pull(proportion) # extracts single numeric value instead of returning df

print("The proportion of flights delayed but arriving on time is:")
delayed_timely
```
...
cat("These are the routes flown by multiple airlines:")
common_routes

# part 2
avg_arr_delay_by_airline <- flights |>
  filter(paste(origin, dest) %in% paste(common_routes$origin, common_routes$dest)) |>
  group_by(origin, dest, carrier) |>
  summarise(avg_arr_delay = mean(arr_delay, na.rm = TRUE), .groups = "drop") |>
  left_join(airlines, by = "carrier")  # Get airline names

print("These are the average arrival delays per route per airline, along with the airline's name:")
avg_arr_delay_by_airline

#part 3
best_worst_airline_per_route <- avg_arr_delay_by_airline |>
  group_by(origin, dest) |>
  summarise(
    worst_airline = carrier[which.max(avg_arr_delay)], # finds the index of the first max avg arr delay
...
print("These are the airlines with the worst and best average arrival delays:")
best_worst_airline_per_route

# part 4

route_greatest_diff <- best_worst_airline_per_route |>
  mutate(delay_diff = max_delay - min_delay) |>
  arrange(desc(delay_diff)) |>
  slice(1)

print("The route with the greatest difference between the best and worst performing airlines is shown below.")
route_greatest_diff

#
flights |>
  filter(
    origin == route_greatest_diff$origin,
    dest == route_greatest_diff$dest
  ) |>
  group_by(carrier) |>
  summarise(
...
print("This shows that a plausible reason for the above-mentioned route having such a large difference between its best (9E) and worst (EV) performing airlines could be flight scheduling cinflicts or congestion as 9E's total flights is only 141 while EV's is 267.")
